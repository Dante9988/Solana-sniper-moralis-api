...
async function buyToken(
  ...
  try {
    ...
    const tokenData = tokenDataFromBondingCurveTokenAccBuffer(
      bondingCurveTokenAccount!.data
    );
    ...
    const buyAmountSolWithSlippage =
      solAmountLamp + (solAmountLamp * SLIPAGE_POINTS) / 10000n;
    ...
    if (isNext) {
      ...
      for (let i = 0; i < next_block_addrs.length; i++) {
        ...
        const transferInstruction = SystemProgram.transfer({
          ...
          lamports: process.env.NEXT_BLOCK_FEE
            ? Number(process.env.NEXT_BLOCK_FEE) * web3.LAMPORTS_PER_SOL
            : 1000000,
        });
        ...
        try {
          ...
          const responseData = await response.json() as { signature: string };
          console.log("responseData", responseData);

          if (response.ok) {
            console.log(
              "Sent transaction with signature",
              `https://solscan.io/tx/${responseData.signature?.toString()}`
            );
            break;
          } else {
            console.error(
      ...
      console.log(
        `Sending transaction for buying ${mint.toString()}, ${new Date().toISOString()}`
      );
