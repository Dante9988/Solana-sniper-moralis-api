...
export const RobinhoodTokenListQuerySchema = z.object({
   ...
   * `trending`: a positive market-cap change over the last hour, largest first.
   */
  lifecycle: z.enum(["all", "bonding", "graduated", "almost-bonded", "trending"]).optional().default("all"),
  /** Defaults: `new` (all/bonding/graduated), `progress` (almost-bonded), `change1h` (trending). */
  sort: z.enum(["new", "marketCap", "liquidity", "progress", "change1h"]).optional(),
